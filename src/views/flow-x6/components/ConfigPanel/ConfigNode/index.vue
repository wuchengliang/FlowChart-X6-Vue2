        ...
        <el-row>
          ...
          <el-col :span="14">
            <el-input
              ...
              v-model="nodeWidth"
              style="width: 100%"
              @change="onNodeWidthHeightChange"
            />
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="8">高度</el-col>
          <el-col :span="14">
            <el-input
              size="mini"
              v-model="nodeHeight"
      ...
      <el-tab-pane v-if="nodeType == 'text'" label="属性" name="3">
        ...
        <el-row>
          ...
          <el-col :span="14">
            <el-select
              size="mini"
              @change="onNodePointCode"
              v-model="nodePointCode"
              placeholder="请选择"
            >
        ...
        <el-row>
          ...
          <el-col :span="14">
            <el-select
              size="mini"
              @change="onNodeFieldName"
              v-model="nodeFieldName"
              placeholder="请选择"
            >
...
export default {
  ...
  watch: {
    id: {
      handler() {
        ...
        if (this.curCel) {
          ...
          this.currentId = this.curCel.id;
          this.positionX = this.curCel?.getPosition().x;
          this.positionY = this.curCel?.getPosition().y;
          this.nodeHeight = this.curCel?.getSize().height;
          this.nodeWidth = this.curCel?.getSize().width;
    ...
    portX: {
      handler() {
        this.curCel?.setPortProp(this.portId, "args", {
          x: this.portX + "%",
          y: this.portY + "%",
        });
      },
  ...
  computed: {
    ...
    positionX: {
      get() {
        return this.$store.state.flow.positionXY.x;
      },
      set(val) {
        this.$store.commit("flow/updatePositionXY", { x: val });
      },
    },
    positionY: {
      get() {
        return this.$store.state.flow.positionXY.y;
      },
      set(val) {
        this.$store.commit("flow/updatePositionXY", { y: val });
      },
    },
  },
  methods: {
    ...
    onNodePositionChange() {
      this.curCel?.setPosition(this.positionX, this.positionY);
      // getPosition
    },
    onNodeWidthHeightChange() {
      this.curCel?.setSize(this.nodeWidth, this.nodeHeight);
      // getPosition
    },
  },
};
